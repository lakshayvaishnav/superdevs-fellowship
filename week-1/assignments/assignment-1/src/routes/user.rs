...
pub async fn sign_up(
    ...
    db: Data<Arc<Mutex<db::Db>>>
) -> impl Responder {
    let mut db = db.lock().unwrap();
    let user = db.users.iter().find(|u| { u.username == request.username });

    match user {
        Some(_) => { HttpResponse::BadRequest().body("User already exists") }
        None => {
...
pub async fn sign_in(
    ...
    match user {
        ...
        None => { HttpResponse::BadRequest().body("User doesnt exist") }
    }
}
